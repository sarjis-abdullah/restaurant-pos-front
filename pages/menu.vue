...
const loadData = async () => {
  try {
    ...
    const { meta, data } = await MenuService.getAll(searchQuery.value);
    list.value = data;
...
const updateRecord = async (id) => {
  try {
    ...
    const res = await MenuService.put(id, updateableRecord.value);
    list.value = list.value.map((item) => {
      if (item.id == id) {
                ...
                <tr v-for="singleData in list" :key="singleData.id">
                  <td class="whitespace-nowrap px-3 py-5 text-sm">
                    <span class="text-gray-900">
                      {{ singleData?.name }}</span>
                  </td>
                  <td class="whitespace-nowrap px-3 py-5 text-sm">
                    <span class="text-gray-900">
                      {{ singleData?.branch_id }}</span>
                  </td>
                  <td class="whitespace-nowrap px-3 py-5 text-sm">
                    <span class="text-gray-900">
                      {{ singleData?.status }}</span>
                  </td>
