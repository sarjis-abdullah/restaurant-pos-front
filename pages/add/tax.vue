...
const rules = computed(() => {
  return {
    rate: { required: helpers.withMessage("Rate is required", required) },
...
const postItem = async () => {
  try {
    loading.value = true;
    await TaxService.create(discountData.value);
...
<template>
  <section class="rounded-lg bg-slate-[#A8A8A8] shadow-lg p-6">
    <header class="hidden md:flex justify-between text-gray-900 mb-3 text-xl">
      <h6 class="hidden md:inline-block capitalize">{{ "Add Tax" }}</h6>
    </header>
      ...
      <section class="grid grid-cols-1 md:grid-cols-3 gap-3">
        ...
        <div class="grid gap-2">
          ...
          <input
            ...
            :placeholder="
              state.type == 'flat'
                ? 'e.g. Discount in flat rate'
                : 'e.g. Discount in percentage rate'
            "
          />
        </div>
      </section>
