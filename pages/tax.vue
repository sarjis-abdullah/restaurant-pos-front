...
const loadData = async () => {
  try {
    ...
    const { meta, data } = await TaxService.getAll(searchQuery.value);
    list.value = data;
...
const updateRecord = async (id) => {
  try {
    ...
    const res = await TaxService.put(id, updateableRecord.value);
    list.value = list.value.map((item) => {
      if (item.id == id) {
